...
works = [
  ...
  { :id => 3, :name => "中級編", :description => "current_userを使って複雑なデータを取得してみよう!",:slug => "intermediate", :order_number => 1}
]
...
chapters = [
  ...
  { :id => 41, :work_id => 3, :name => "current_userがいいねした全ての投稿の中で最も多くコメントされたものを取得する", :slug => "intermediate", :order_number => 2},
  { :id => 42, :work_id => 3, :name => "current_userがコメントした投稿の中で最も多くいいねされたものを取得する", :slug => "intermediate", :order_number => 2}
...
practices = [
  ...
  { :id => 42, :work_id => 3, :chapter_id =>42, :user_id => 5, :title => "current_userがコメントした投稿の中で最も多くいいねされた投稿を取得する", :question => 'Andersonがcurrent_userです。Andersonがいいねした全ての投稿の中で最も多くコメントされたものを取得するためには、どのようにコードを書くべきでしょうか？', :answer => 'current_user.comments.map(&:post).max_by { |post| post.likes.count }', :order_number => 3}
]
